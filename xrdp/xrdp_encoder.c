...
build_enc_h264_avc444_yuv420_and_chroma420_stream(struct xrdp_encoder *self, XRDP_ENC_DATA *enc)
{
    ...
    else
    {
        ...
        error = xrdp_encoder_x264_encode(self->codec_handle_x264,
                                         0, 0, 0,
                                         enc->u.sc.width, enc->u.sc.height,
                                         enc->u.sc.width, enc->u.sc.height, /* twidth, theight */
                                         0, enc_gfx_cmd->data,
    ...
    else
    {
        ...
        error = xrdp_encoder_openh264_encode(self->codec_handle_openh264, 0,
                                             enc->u.sc.width, enc->u.sc.height, 0,
                                             enc->u.sc.data
                                                 + (enc->u.sc.height * enc->u.sc.width) * 3 / 2,
                                             s->p, &out_data_bytes);
#endif
    }
    if (error != 0)
...
#if 0
    g_writeln("comp_bytes_pre %d out_data_bytes %d comp_bytes_pre %d out_data_bytes %d",
              comp_bytes_pre, out_data_bytes, comp_bytes_pre, out_data_bytes);
...
build_enc_h264_avc444_chroma420_stream(struct xrdp_encoder *self, XRDP_ENC_DATA *enc, XRDP_ENC_DATA_DONE *enc_done)
{
    ...
    {
        /* already compressed */
        uint8_t *ud = (uint8_t *) (enc->u.sc.data + enc_done->out_data_bytes + 4);
        int cbytes = ud[0] | (ud[1] << 8) | (ud[2] << 16) | (ud[3] << 24);
        if ((cbytes < 1) || (cbytes > out_data_bytes))
        {
            LOG(LOG_LEVEL_INFO, "process_enc_h264: bad h264 bytes %d", cbytes);
        ...
        LOG(LOG_LEVEL_DEBUG,
            "process_enc_h264: already compressed and size is %d", cbytes);
        out_data_bytes = cbytes;
        g_memcpy(s->p, enc->u.sc.data + enc_done->out_data_bytes + 8, out_data_bytes);
    }
    else
    {
#if defined(XRDP_VANILLA_NVIDIA_CODEC)
        error = xrdp_encoder_nvenc_encode(self->codec_handle_nvenc, 0,
                                    enc->u.sc.width, enc->u.sc.height, 0,
                                    enc->u.sc.data + (enc->u.sc.height * enc->u.sc.width) * 3 / 2,
                                    s->p, &out_data_bytes);
#elif defined(XRDP_X264)
        error = xrdp_encoder_x264_encode(self->codec_handle_x264,
                                         0, 0, 0,
...
process_enc_h264(struct xrdp_encoder *self, XRDP_ENC_DATA *enc)
{
    ...
    enc_done->rect = calculate_bounding_box(enc->u.sc.drects, enc->u.sc.num_drects);
